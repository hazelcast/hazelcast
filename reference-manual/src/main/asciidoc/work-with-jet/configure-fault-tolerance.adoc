...
with the at-least-once guarantee, after a job restart you may get
duplicates in the output. As opposed to doubly processing an input item,
this is more benign because it just means getting the exact same result
twice. If you use a source with the at-least-once guarantee, some events
...
Also note that duplicate event processing can occur only in the case of
a failure. If there is no failure or if the job is restarted due to a
normal member shutdown or addition, even jobs in this mode do
