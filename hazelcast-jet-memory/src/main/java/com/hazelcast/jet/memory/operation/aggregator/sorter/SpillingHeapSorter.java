 ...
 * Sorts spilled data.
 */
public class SpillingHeapSorter extends AbstractHeapSorter<SpillingKeyValueWriter> {
    public SpillingHeapSorter(
            int chunkSize, SortOrder direction, ObjectHolder<Comparator> comparatorHolder,
            Comparator comparator, Accumulator binaryFunctor, MemoryBlock temporaryMemoryBlock,
            boolean useBigEndian
    ) {
        super(chunkSize, direction, comparatorHolder, comparator, binaryFunctor, temporaryMemoryBlock, useBigEndian);
    }
    ...
    protected void outputSlot(InputsCursor iterator, int inputId) {
        output.writeSlotHeader(iterator.partitionId(inputId), iterator.getHashCode(inputId), 1);
    }

    @Override
    protected void outputSegment(int sourceId, long recordsCount) {
        output.writeSegmentHeader(sourceId, recordsCount);
    }
