...
/**
 * Accumulates long values into a long value.
 */
public abstract class LongAccumulator implements Accumulator {
    @Override
    public void accept(
        ...
        long oldValue = EndiannessUtil.readLong(CUSTOM_ACCESS,
                accMemoryAccessor, oldDataAddress, useBigEndian);
        long newValue = EndiannessUtil.readLong(CUSTOM_ACCESS,
                newMemoryAccessor, newDataAddress, useBigEndian);
        EndiannessUtil.writeLong(CUSTOM_ACCESS,
                accMemoryAccessor, oldDataAddress, apply(oldValue, newValue), useBigEndian);
    }

    protected abstract long apply(long oldValue, long newValue);
}
