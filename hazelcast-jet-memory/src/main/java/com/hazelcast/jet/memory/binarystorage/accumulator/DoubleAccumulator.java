...
/**
 * Accumulates double values into a double value.
 */
public abstract class DoubleAccumulator implements Accumulator {
    ...
    public void accept(
        ...
        double oldValue = EndiannessUtil.readDouble(CUSTOM_ACCESS,
                accMemoryAccessor, oldDataAddress, useBigEndian);
        double newValue = EndiannessUtil.readDouble(CUSTOM_ACCESS,
                newMemoryAccessor, newDataAddress, useBigEndian);
        EndiannessUtil.writeDouble(CUSTOM_ACCESS,
                accMemoryAccessor, oldDataAddress, apply(oldValue, newValue), useBigEndian);
    }

    protected abstract double apply(double oldValue, double newValue);
}
