...
public class MinusTest extends ExpressionTestBase {
    ...
    private static RelDataType[] expectedTypes(Operand[] operands) {
        ...
        if (lhs.typeName() == BOOLEAN || rhs.typeName() == BOOLEAN) {
            // BOOLEAN - whatever, whatever - BOOLEAN
            return null;
        }
        ...
        if (isNull(lhsType) && isNull(rhsType)) {
            RelDataType nullType = TYPE_FACTORY.createSqlType(NULL);
            return new RelDataType[]{nullType, nullType, nullType};
        }

        // Infer types.

        return expectedNumericTypes(HazelcastReturnTypes::binaryIntegerMinus, lhs, rhs, lhsType, rhsType);
    }
