...
public final class IndexResolver {
    ...
    private static IndexComponentCandidate prepareSingleColumnCandidateIsNull(RexNode exp, RexNode operand) {
        ...
        QueryDataType type = CalciteUtils.map(operand.getType().getSqlTypeName());
    ...
    private static RexNode removeCastIfPossible(RexNode node) {
        if (node.getKind() == SqlKind.CAST) {
            ...
            if (from instanceof RexInputRef) {
                ...
                QueryDataTypeFamily fromFamily = CalciteUtils.map(fromType.getSqlTypeName()).getTypeFamily();
                QueryDataTypeFamily toFamily = CalciteUtils.map(toType.getSqlTypeName()).getTypeFamily();
