...
package com.hazelcast.sql.impl.calcite.rule.physical.agg;
...
public class AggregatePhysicalRule extends RelOptRule {
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        Collection<RelNode> physicalInputs = RuleUtils.getPhysicalRelsFromSubset(RuleUtils.toPhysicalInput(input));
    ...
    private static RelNode optimize(AggregateLogicalRel logicalAgg, RelNode physicalInput) {
        ...
        if (localAggDistribution.isCollocated()) {
            RelTraitSet traitSet = RuleUtils.traitPlus(
    ...
    private static RelNode optimizeNonCollocated(
        ...
        RelTraitSet localTraitSet = RuleUtils.traitPlus(
            physicalInput.getTraitSet(),
            localAggCollation.getCollation(),
        ...
        if (broadcast) {
            exchange = new BroadcastExchangePhysicalRel(
                cluster,
                RuleUtils.toPhysicalConvention(cluster.traitSet(), DistributionTrait.REPLICATED_DIST),
                localAgg
            );
        } else {
            ...
            exchange = new UnicastExchangePhysicalRel(
                cluster,
                RuleUtils.toPhysicalConvention(cluster.traitSet(), distribution),
                localAgg,
                hashFields
            );
        }
