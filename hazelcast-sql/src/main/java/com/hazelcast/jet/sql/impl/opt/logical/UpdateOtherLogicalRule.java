...
/**
 * A rule to match a TableModify[operation=update], with neither Scan nor Values
 * as input - these are handled by {@link UpdateWithScanLogicalRule} and {@link
 * UpdateWithValuesLogicalRule} respectively.
 */
@Value.Enclosing
class UpdateOtherLogicalRule extends RelRule<RelRule.Config> {
    ...
    interface Config extends RelRule.Config {
        RelRule.Config DEFAULT = ImmutableUpdateOtherLogicalRule.Config.builder()
                .description(UpdateOtherLogicalRule.class.getSimpleName())
        ...
        default RelOptRule toRule() {
            return new UpdateOtherLogicalRule(this);
        }
    }

    UpdateOtherLogicalRule(RelRule.Config config) {
        super(config);
    }
