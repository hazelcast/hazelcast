 ...
 */
@Value.Enclosing
class UpdateNewLogicalRule extends RelRule<RelRule.Config> {
    ...
    interface Config extends RelRule.Config {
        ...
        default RelOptRule toRule() {
            return new UpdateNewLogicalRule(this);
        }
    }

    UpdateNewLogicalRule(RelRule.Config config) {
        super(config);
    }
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        if (input2 instanceof FullScanLogicalRel) {
            FullScanLogicalRel scan = (FullScanLogicalRel) input2;
            ...
            UpdateLogicalRel rel = new UpdateLogicalRel(
                    ...
                    update.getTable(),
                    update.getCatalogReader(),
                    OptUtils.toLogicalInput(input2),
                    requireNonNull(update.getUpdateColumnList()),
                    requireNonNull(update.getSourceExpressionList()),
                    update.isFlattened(),
                    null
