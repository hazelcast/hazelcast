...
public class MapService implements ManagedService, MigrationAwareService, MembershipAwareService,
    ...
    public void prepare(String txnId, int partitionId) throws TransactionException {
        ...
        try {
            nodeEngine.getOperationService().takeBackups(SERVICE_NAME, new MapTxnBackupPrepareOperation(txnLog), partitionId, 0,
                    maxBackupCount, 60);
        } catch (Exception e) {
    ...
    public void commit(String txnId, int partitionId) throws TransactionException {
        ...
        int maxBackupCount = 1; //txnLog.getMaxBackupCount();
        try {
            nodeEngine.getOperationService().takeBackups(SERVICE_NAME, new MapTxnBackupCommitOperation(txnId), partitionId, 0,
                    maxBackupCount, 60);
    ...
    public void rollback(String txnId, int partitionId) throws TransactionException {
        ...
        int maxBackupCount = 1; //txnLog.getMaxBackupCount();
        try {
            nodeEngine.getOperationService().takeBackups(SERVICE_NAME, new MapTxnBackupRollbackOperation(txnId), partitionId, 0,
                    maxBackupCount, 60);
        } catch (Exception e) {
            throw new TransactionException(e);
        }
