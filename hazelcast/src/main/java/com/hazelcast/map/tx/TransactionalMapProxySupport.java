...
public abstract class TransactionalMapProxySupport extends AbstractDistributedObject<MapService>
    ...
    protected List<Map.Entry<Data, Data>> entrySetInternal() {
        final NodeEngine nodeEngine = getNodeEngine();
        try {
            Map<Integer, Object> results = nodeEngine.getOperationService()
                    .invokeOnAllPartitions(
            ...
            ArrayList<Map.Entry<Data, Data>> entrySet = new ArrayList<Map.Entry<Data, Data>>();
            for (Object result : results.values()) {
                entrySet.addAll(((MapEntrySet) getService()
