...
public class TxnRollbackBackupOperation extends KeyBasedMapOperation implements BackupOperation {
    ...
    protected TxnRollbackBackupOperation(String name, Data dataKey, String lockOwner, int lockThreadId) {
        super(name, dataKey);
    ...
    public void run() throws Exception {
        if (recordStore.isLocked(getKey()) && !recordStore.unlock(getKey(), lockOwner, lockThreadId)) {
            throw new TransactionException("Lock is not owned by the transaction !" + lockOwner + " thId: " + lockThreadId);
        }
    }
