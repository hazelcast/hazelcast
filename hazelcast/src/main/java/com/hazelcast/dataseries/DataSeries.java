...
public interface DataSeries<K, V> extends DistributedObject {
    ...
    long append(int partitionId, byte[] value);

    ICompletableFuture<Long> appendAsync(int partitionId, byte[] value);

    byte[] get(int partitionId, long sequence);

    ICompletableFuture<byte[]> getAsync(int partitionId, long sequenceId);
