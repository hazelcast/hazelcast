...
public class ClusterJoinManager {
    ...
    public void handleMasterResponse(Address masterAddress, Address callerAddress) {
        ...
        try {
            ...
            Connection conn = node.server.getConnectionManager(MEMBER).get(currentMaster);
    ...
    private void setMasterAndJoin(Address masterAddress) {
        ...
        node.server.getConnectionManager(MEMBER).getOrConnect(masterAddress);
    ...
    private boolean checkIfJoinRequestFromAnExistingMember(JoinMessage joinMessage, ServerConnection connection) {
        ...
        if (clusterService.isMaster() || target.equals(clusterService.getMasterAddress())) {
            ...
            ServerConnection existing = node.server.getConnectionManager(MEMBER).get(target);
            if (existing != connection) {
                if (existing != null) {
                    existing.close(msg, null);
                }
                node.server.getConnectionManager(MEMBER).register(target, connection);
            }
        }
        return true;
    }
