...
public abstract class AbstractJoiner
        ...
        if (clusterService.isJoined()) {
            ...
            while (checkCount++ < connectAllWaitSeconds) {
                ...
                for (Member member : members) {
                    if (!member.localMember() && node.server.getConnectionManager(MEMBER)
    ...
    private SplitBrainJoinMessage sendSplitBrainJoinMessage(Address target, SplitBrainJoinMessage request) {
        ...
        Connection conn = node.server.getConnectionManager(MEMBER).getOrConnect(target, true);
        ...
        while (conn == null) {
            ...
            conn = node.server.getConnectionManager(MEMBER).get(target);
        }
