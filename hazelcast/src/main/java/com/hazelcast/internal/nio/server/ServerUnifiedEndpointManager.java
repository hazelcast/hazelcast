...
class ServerUnifiedEndpointManager
        extends ServerEndpointManager {

    ServerUnifiedEndpointManager(NetworkingService root, EndpointConfig endpointConfig,
    ...
    Set<ServerConnection> getTextConnections() {
        Set<ServerConnection> connections = activeConnections.isEmpty()
                ? Collections.emptySet()
                : new HashSet<>(activeConnections.size());
