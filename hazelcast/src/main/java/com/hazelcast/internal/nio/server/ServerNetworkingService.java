...
public final class ServerNetworkingService implements NetworkingService<ServerConnection> {
    ...
    private final AtomicReference<ServerAcceptor> acceptorRef = new AtomicReference<>();

    private volatile boolean live;

    ServerNetworkingService(Config config, IOService ioService,
    ...
    public ServerNetworkingService(Config config, IOService ioService,
                                   ServerSocketRegistry registry,
                                   LoggingService loggingService,
        ...
        this.logger = loggingService.getLogger(ServerNetworkingService.class);
        ...
        if (registry.holdsUnifiedSocket()) {
            unifiedEndpointManager = new ServerUnifiedEndpointManager(this, null, channelInitializerProvider,
    ...
    private EndpointManager<ServerConnection> newEndpointManager(IOService ioService,
                                                                 EndpointConfig endpointConfig,
                                                                 ChannelInitializerProvider channelInitializerProvider,
                                                                 LoggingService loggingService,
                                                                 HazelcastProperties properties,
                                                                 Set<ProtocolType> supportedProtocolTypes) {
        return new ServerEndpointManager(this, endpointConfig, channelInitializerProvider, ioService, loggingService,
    ...
    public synchronized void stop() {
        ...
        if (unifiedEndpointManager != null) {
            ...
            for (EndpointManager endpointManager : endpointManagers.values()) {
                ((ServerEndpointManager) endpointManager).reset(false);
            }
        }
    ...
    public synchronized void shutdown() {
        ...
        if (unifiedEndpointManager != null) {
            ...
            for (EndpointManager endpointManager : endpointManagers.values()) {
                ((ServerEndpointManager) endpointManager).reset(true);
            }
        }
    }
     ...
     * Under unified endpoint environments, this will return the respective view of the {@link ServerUnifiedEndpointManager}
    ...
    public EndpointManager<ServerConnection> getEndpointManager(EndpointQualifier qualifier) {
        EndpointManager<ServerConnection> mgr = endpointManagers.get(qualifier);
        if (mgr == null) {
    ...
    EndpointManager<ServerConnection> getUnifiedOrDedicatedEndpointManager(EndpointQualifier qualifier) {
    ...
    private void startAcceptor() {
        ...
        acceptorRef.set(new ServerAcceptor(registry, this, ioService).start());
    }

    private void shutdownAcceptor() {
        ServerAcceptor acceptor = acceptorRef.get();
        if (acceptor != null) {
            acceptor.shutdown();
    ...
    private static final class RefreshNetworkStatsTask implements Runnable {
        ...
        RefreshNetworkStatsTask(ConcurrentMap<EndpointQualifier, EndpointManager<ServerConnection>> endpointManagers) {
        ...
        public void run() {
            for (ProtocolType type : ProtocolType.valuesAsSet()) {
                ...
                for (EndpointManager endpointManager : endpointManagers.values()) {
                    ServerEndpointManager tcpIpEndpointManager = (ServerEndpointManager) endpointManager;
    ...
    private static final class MetricsProvider implements DynamicMetricsProvider {
        private final AtomicReference<ServerAcceptor> acceptorRef;
        private final ConcurrentMap<EndpointQualifier, EndpointManager<ServerConnection>> endpointManagers;
        private final ServerUnifiedEndpointManager unifiedEndpointManager;

        private MetricsProvider(AtomicReference<ServerAcceptor> acceptorRef,
                                ConcurrentMap<EndpointQualifier, EndpointManager<ServerConnection>> endpointManagers,
                                ServerUnifiedEndpointManager unifiedEndpointManager) {
        ...
        public void provideDynamicMetrics(MetricDescriptor descriptor, MetricsCollectionContext context) {
            ...
            ServerAcceptor acceptor = this.acceptorRef.get();
            if (acceptor != null) {
                acceptor.provideDynamicMetrics(descriptor.copy(), context);
            }

            for (EndpointManager<ServerConnection> manager : this.endpointManagers.values()) {
