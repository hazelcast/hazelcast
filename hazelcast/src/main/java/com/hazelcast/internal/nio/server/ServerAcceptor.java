 ...
 *
 * The {@link ServerAcceptor} and {@link ServerConnector} are 2 sides of the same coin. The {@link ServerConnection} take care
 * of the 'client' side of a connection and the {@link ServerAcceptor} is the 'server' side of a connection (each connection
 * has a client and server-side
 */
public class ServerAcceptor implements DynamicMetricsProvider {
    ...
    private final ServerNetworkingService networkingService;
    ...
    ServerAcceptor(ServerSocketRegistry registry, ServerNetworkingService networkingService, IOService ioService) {
    ...
    /**
     * A probe that measure how long this {@link ServerAcceptor} has not received any events.
     *
    ...
    private final class AcceptorIOThread extends Thread {
        ...
        private void newConnection(final EndpointQualifier qualifier, SocketChannel socketChannel) throws IOException {
            ServerEndpointManager endpointManager = (ServerEndpointManager)
        ...
        private void newConnection0(ServerEndpointManager endpointManager, Channel channel) {
            try {
