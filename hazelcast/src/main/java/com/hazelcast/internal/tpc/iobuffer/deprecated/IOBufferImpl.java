 ...
 */
public class IOBufferImpl {

    public IOBufferImpl next;
    ...
    public IOBufferImpl(int size) {
        this(size, false);
    }

    public IOBufferImpl(int size, boolean direct) {
        //todo: allocate power of 2.
        this.buff = direct ? ByteBuffer.allocateDirect(size) : ByteBuffer.allocate(size);
    }

    public IOBufferImpl(ByteBuffer buffer) {
        this.buff = buffer;
    }
