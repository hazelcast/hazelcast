...
public final class CompatibilityBindHandler {
    ...
    public CompatibilityBindHandler(TcpIpEndpointManager tcpIpEndpointManager, IOService ioService, ILogger logger,
    ...
    public void process(Packet packet) {
        ...
        if (connection.setBinding()) {
            CompatibilityBindMessage bindMessage = (CompatibilityBindMessage) bind;
    ...
    private synchronized boolean bind(TcpIpConnection connection, CompatibilityBindMessage bindMessage) {
        if (logger.isFinestEnabled()) {
            logger.finest("Binding " + connection + ", complete message is " + bindMessage);
        }

        Map<ProtocolType, Collection<Address>> remoteAddressesPerProtocolType
                = bindMessage.getLocalAddresses();
        ...
        assert (tcpIpEndpointManager.getEndpointQualifier() != EndpointQualifier.MEMBER
                || connection.getType().equals(ConnectionType.MEMBER))
                : "When handling MEMBER connections, connection type"
                + " must be already set";
        boolean isMemberConnection = (connection.getType().equals(ConnectionType.MEMBER)
    ...
    private synchronized boolean bind0(TcpIpConnection connection, Address remoteEndpoint,
        ...
        if (reply) {
            CompatibilityBindRequest bindRequest
                    = new CompatibilityBindRequest(logger, ioService, connection, remoteEndpoint, false);
            bindRequest.send();
        }
