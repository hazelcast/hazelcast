...
public abstract class AbstractNonBlockingSocketWriter<C extends Connection>
            ...
            public void run() {
                try {
                    if (writeHandler == null) {
                        writeHandler = createWriterHandler(protocol);
                    }
                } catch (Throwable t) {
                    onFailure(t);
                } finally {
    ...
    protected abstract WriteHandler createWriterHandler(String protocol) throws IOException;

    protected void configureBuffers(int size, boolean direct) {
        outputBuffer = IOUtil.newByteBuffer(size, direct);
    }
    ...
    public void handle() throws Exception {
        ...
        if (writeHandler == null) {
            writeHandler = createWriterHandler(CLUSTER);
        }
