...
public abstract class AbstractCacheRecordStore<R extends CacheRecord, CRM extends SampleableCacheRecordMap<Data, R>>
    ...
    protected boolean hasReplicaAddress(int partitionId, int replicaIndex) {
        return nodeEngine.getPartitionService().getPartition(partitionId).getReplicaAddress(replicaIndex) != null;
    }
    ...
    private int evictExpiredInternal(int maxIterationCount, long now) {
        ...
        while (expirationIterator.hasNext()) {
            ...
            records.add(record);
            processedCount++;
            if (processedCount >= maxIterationCount) {
                break;
            }
        }
        int evictedCount = 0;
        while (!records.isEmpty()) {
            ...
            boolean expired = processExpiredEntry(record.getKey(), record.getValue(), now);
            if (expired) {
                evictedCount++;
            }
        }
