...
public abstract class AbstractCacheService implements ICacheService, PreJoinAwareService, PostJoinAwareService,
    ...
    // protect concurrent creation and destruction of same Cache
    protected final ContextMutexFactory cacheLifecycleMutexFactory = new ContextMutexFactory();
    ...
    public DistributedObject createDistributedObject(String cacheNameWithPrefix) {
        try {
            ...
            if (cacheNameWithPrefix.equals(SETUP_REF)) {
                ...
                ContextMutexFactory.Mutex mutex = cacheLifecycleMutexFactory.mutexFor(cacheNameWithPrefix);
                try {
                    synchronized (mutex) {
    ...
    @Override
    public ContextMutexFactory.Mutex getLifecycleMutex(String cacheNameWithPrefix) {
        return cacheLifecycleMutexFactory.mutexFor(cacheNameWithPrefix);
    }
