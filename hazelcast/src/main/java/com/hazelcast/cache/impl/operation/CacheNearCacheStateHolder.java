...
public class CacheNearCacheStateHolder implements IdentifiedDataSerializable {
    ...
    void prepare(CachePartitionSegment segment) {
        ...
        while (iter.hasNext()) {
            ...
            String mapName = cacheRecordStore.getName();

            cacheNameSequencePairs.add(mapName);
            cacheNameSequencePairs.add(metaData.currentSequence(mapName, partitionId));
        }
    }
    ...
    void applyState() {
        ...
        for (int i = 0; i < cacheNameSequencePairs.size(); ) {
            String mapName = (String) cacheNameSequencePairs.get(i++);
