...
public final class OperationExecutorImpl implements OperationExecutor, MetricsProvider {
    ...
    private void execute(Object task, int partitionId, boolean priority) {
        if (partitionId < 0) {
            ...
            runOrElseExecuteNotNested((Packet)task, Thread.currentThread(), runnerRef);
        } else {
    ...
    private void runOrElseExecuteNotNested(Packet op, Thread currentThread, OperationRunnerReference runnerRef) {
        int partitionId = op.getPartitionId();
