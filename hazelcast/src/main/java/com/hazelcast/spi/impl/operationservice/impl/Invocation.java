...
public abstract class Invocation implements OperationResponseHandler, Runnable {
    ...
    // been running.
    final long firstInvocationTimeMs = Clock.currentTimeMillis();
    ...
    Invocation(OperationServiceImpl operationService, Operation op, int tryCount, long tryPauseMillis, long callTimeout,
               boolean deserialize, ExecutionCallback callback) {
    ...
    HeartbeatTimeout detectTimeout(long heartBeatTimeoutMs) {
        ...
        // todo: we are looking at the op.callTimeout; but we are not checking the callTimeout of the invocation

        long callTimeoutMs = op.getCallTimeout();
        if (callTimeoutMs <= 0 || callTimeoutMs == Long.MAX_VALUE) {
            return NO__CALL_TIMEOUT_DISABLED;
        }

        // a call is always allowed to execute as long as its own call timeout.
        long callExpirationTimeMs = op.getInvocationTime() + callTimeoutMs;
        if (callExpirationTimeMs > currentTimeMs) {
        ...
        long lastHeartbeatMs = this.lastHeartbeatMs;
        long heartBeatExpirationTimeMs = lastHeartbeatMs == 0
                ? callTimeoutMs + op.getInvocationTime() + heartBeatTimeoutMs
                : lastHeartbeatMs + heartBeatTimeoutMs;

        if (heartBeatExpirationTimeMs > currentTimeMs) {
    ...
    enum HeartbeatTimeout {
        NO__CALL_TIMEOUT_DISABLED,
        NO__RESPONSE_AVAILABLE,
        NO__HEARTBEAT_TIMEOUT_NOT_EXPIRED,
        NO__CALL_TIMEOUT_NOT_EXPIRED,
        YES
    }
    ...
    public String toString() {
        ...
        return "Invocation{"
                ...
                + ", firstInvocationTimeMs=" + firstInvocationTimeMs
                + ", firstInvocationTime='" + new Date(firstInvocationTimeMs) + "'"
                + ", lastHeartbeatMs=" + lastHeartbeatMs
                + ", lastHeartbeatTime='" + new Date(lastHeartbeatMs) + "'"
                + ", target=" + invTarget
