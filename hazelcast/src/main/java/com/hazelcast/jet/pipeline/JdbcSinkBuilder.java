...
public class JdbcSinkBuilder<T> {
    ...
    private ExternalDataLinkRef externalDataLinkRef;
    ...
    /**
     * Sets the reference to the configured external data link of {@link ExternalDataLinkRef} from which
     * the instance of the {@link javax.sql.DataSource} will be retrieved.
     * <p>
     * Example:
     * <p>
     * (Prerequisite) External data link configuration:
     * <pre>{@code
     ...
     *                         .externalDataLinkRef(externalDataLinkRef("my-jdbc-data-link"))
     *                         .bindFn((stmt, item1) -> {
     ...
     * </p>
     *
     * @param externalDataLinkRef the reference to the configured external data link
     * @return this instance for fluent API
     * @since 5.2
     */
    @Nonnull
    @Beta
    public JdbcSinkBuilder<T> externalDataLinkRef(ExternalDataLinkRef externalDataLinkRef) {
        this.dataSourceSupplier = null;
        this.jdbcUrl = null;
    ...
    @Nonnull
    public Sink<T> build() {
        if (dataSourceSupplier == null && jdbcUrl == null && externalDataLinkRef == null) {
            throw new IllegalStateException("Neither jdbcUrl() nor dataSourceSupplier() nor externalDataLinkRef() set");
        }
        if (jdbcUrl != null) {
        ...
        return Sinks.fromProcessor("jdbcSink",
                SinkProcessors.writeJdbcP(updateQuery, externalDataLinkRef, bindFn,
                        exactlyOnce, batchLimit));
