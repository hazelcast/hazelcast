...
public interface ProcessorMetaSupplier extends Serializable {
    ...
    /**
     * The value of localParallelism with the meaning
     ...
     * <p>
     * The parallelism will be overridden if the {@code Vertex#localParallelism(int)} is
        ...
        class SpecificMemberPms implements ProcessorMetaSupplier, DataSerializable {
            ...
            public Function<? super Address, ? extends ProcessorSupplier> get(@Nonnull List<Address> addresses) {
                if (!addresses.contains(memberAddress)) {
                    throw new HazelcastException("Cluster does not contain the required member: " + memberAddress);
                }
