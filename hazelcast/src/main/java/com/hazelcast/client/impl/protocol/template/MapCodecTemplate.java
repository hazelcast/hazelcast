...
public interface MapCodecTemplate {
    ...
    @Request(id = 13, retryable = false, response = ResponseMessageConst.DATA)
    ...
    @Request(id = 21, retryable = false, response = ResponseMessageConst.DATA)
    ...
    @Request(id = 25, retryable = false, response = ResponseMessageConst.STRING, event = EventResponseMessageType.ITEM_EVENT)
    void addEntryListenerToKeyWithPredicate(String name, Data key, Data predicate, boolean includeValue);

    @Request(id = 26, retryable = false, response = ResponseMessageConst.STRING, event = EventResponseMessageType.ITEM_EVENT)
    void addEntryListenerWithPredicate(String name, Data predicate, boolean includeValue);

    @Request(id = 27, retryable = false, response = ResponseMessageConst.STRING, event = EventResponseMessageType.ITEM_EVENT)
    void addEntryListenerToKey(String name, Data key, boolean includeValue);

    @Request(id = 28, retryable = false, response = ResponseMessageConst.STRING, event = EventResponseMessageType.ITEM_EVENT)
    ...
    @Request(id = 30, retryable = false, response = ResponseMessageConst.DATA)
    ...
    @Request(id = 31, retryable = false, response = ResponseMessageConst.STRING,
             event = EventResponseMessageType.PARTITION_LOST_EVENT)
    void addPartitionLostListener(String name);

    @Request(id = 32, retryable = false, response = ResponseMessageConst.DATA)
    void removePartitionLostListener(String name, String registrationId);

    @Request(id = 33, retryable = false, response = ResponseMessageConst.ENTRY_VIEW)
    void getEntryView(String name, Data key, long threadId);

    @Request(id = 34, retryable = false, response = ResponseMessageConst.DATA)
    void evict(String name, Data key, long threadId);

    @Request(id = 35, retryable = false, response = ResponseMessageConst.DATA)
    void evictAll(String name);

    @Request(id = 36, retryable = false, response = ResponseMessageConst.DATA)
    void loadAll(String name, boolean replaceExistingValues);

    @Request(id = 37, retryable = false, response = ResponseMessageConst.DATA)
    void loadGivenKeys(String name, List<Data> keys, boolean replaceExistingValues);

    @Request(id = 38, retryable = false, response = ResponseMessageConst.DATA)
    void keySet(String name);

    @Request(id = 39, retryable = false, response = ResponseMessageConst.DATA)
    void getAll(String name, Set<Data> keys);

    @Request(id = 40, retryable = false, response = ResponseMessageConst.DATA)
    void values(String name);

    @Request(id = 41, retryable = false, response = ResponseMessageConst.DATA)
    void entrySet(String name);

    @Request(id = 42, retryable = false, response = ResponseMessageConst.DATA)
    void keySetWithPredicate(String name, Data predicate);

    @Request(id = 43, retryable = false, response = ResponseMessageConst.DATA)
    void valuesWithPredicate(String name, Data predicate);

    @Request(id = 44, retryable = false, response = ResponseMessageConst.DATA)
    void entriesWithPredicate(String name, Data predicate);

    @Request(id = 45, retryable = false, response = ResponseMessageConst.DATA)
    void addIndex(String name, String attribute, boolean ordered);

    @Request(id = 46, retryable = false, response = ResponseMessageConst.DATA)
    void size(String name);

    @Request(id = 47, retryable = false, response = ResponseMessageConst.DATA)
    void isEmpty(String name);

    @Request(id = 48, retryable = false, response = ResponseMessageConst.DATA)
    void putAll(String name, List<Data> keys, List<Data> values);

    @Request(id = 49, retryable = false, response = ResponseMessageConst.DATA)
    ...
    @Request(id = 55, retryable = false, response = ResponseMessageConst.DATA)
