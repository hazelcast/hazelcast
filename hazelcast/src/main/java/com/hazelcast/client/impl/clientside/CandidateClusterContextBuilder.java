...
final class CandidateClusterContextBuilder {

    private CandidateClusterContextBuilder() {
    }

    public static Collection<CandidateClusterContext> build(List<ClientConfig> configs,
        ...
        for (ClientConfig config : configs) {
            ...
            SocketInterceptor interceptor = initSocketInterceptor(networkConfig.getSocketInterceptorConfig(), clientExtension);
            ...
            DiscoveryService discoveryService = initDiscoveryService(config, properties, loggingService);
            AddressProvider provider;
            if (externalAddressProvider != null) {
                provider = externalAddressProvider;
            } else {
                provider = createAddressProvider(config, discoveryService, properties, loggingService, clusterService);
            }
    ...
    private static AddressProvider createAddressProvider(ClientConfig clientConfig, DiscoveryService discoveryService,
        ...
        boolean discoverySpiEnabled = discoverySpiEnabled(networkConfig, properties);
        ...
        if (hazelcastCloudEnabled) {
            ...
            return new RemoteAddressProvider(() -> discoverAddresses(discoveryService),
                    usePublicAddress(clientConfig, properties));
        }
    ...
    private static Map<Address, Address> discoverAddresses(DiscoveryService discoveryService) {
    ...
    private static boolean discoverySpiEnabled(ClientNetworkConfig networkConfig, HazelcastProperties properties) {
    ...
    private static boolean usePublicAddress(ClientConfig config, HazelcastProperties properties) {
    ...
    private static void isDiscoveryConfigurationConsistent(boolean addressListProvided, boolean awsDiscoveryEnabled,
    ...
    private static DiscoveryService initDiscoveryService(ClientConfig config, HazelcastProperties properties,
                                                         LoggingService loggingService) {
    ...
    private static boolean isEmptyDiscoveryStrategies(DiscoveryService discoveryService) {
    ...
    private static SocketInterceptor initSocketInterceptor(SocketInterceptorConfig sic, ClientExtension clientExtension) {
    ...
    private static int getConnectionTimeoutMillis(ClientNetworkConfig networkConfig) {
