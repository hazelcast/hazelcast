...
public class AddListenerResultParameters {

    public static final ClientMessageType TYPE = ClientMessageType.ADD_LISTENER_RESULT;
    public String registrationId;

    private AddListenerResultParameters(ClientMessage flyweight) {
        registrationId = flyweight.getStringUtf8();
    }

    public static AddListenerResultParameters decode(ClientMessage flyweight) {
        return new AddListenerResultParameters(flyweight);
    }
    ...
    public static int calculateDataSize(String registrationId) {
        return ClientMessage.HEADER_SIZE
                + ParameterUtil.calculateStringDataSize(registrationId);
    }
}
