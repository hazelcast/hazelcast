...
import com.hazelcast.dataset.impl.aggregation.ExecuteAggregationOperationFactory;
...
public class PreparedAggregation<E> {
    ...
    public PreparedAggregation(OperationService operationService, String name, String preparedId) {
    ...
    /**
     * Executes the whole aggregation on the partition thread (can be very problematic with large datasets)
     *
     * @param bindings
     * @return
     */
    ...
    private E execute(Map<String, Object> bindings, boolean forkJoin) {
        try {
            Map<Integer, Object> result = operationService.invokeOnAllPartitions(
                    DataSetService.SERVICE_NAME, new ExecuteAggregationOperationFactory(name, preparedId, bindings, forkJoin));
