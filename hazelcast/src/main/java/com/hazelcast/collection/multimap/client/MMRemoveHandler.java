...
public class MMRemoveHandler extends MultiMapCommandHandler{
    public MMRemoveHandler(CollectionService collectionService) {
    ...
    protected Protocol processCall(ObjectMultiMapProxy proxy, Protocol protocol) {
        ...
        if(protocol.buffers.length > 1){
            ...
            boolean removed = proxy.remove(key, value);
            return protocol.success(String.valueOf(removed));
        }
        Collection<Object> result = proxy.remove(key);
        return success(proxy, protocol, result);
