...
public class CountDownLatchSplitBrainProtectionWriteTest extends AbstractSplitBrainProtectionTest {
    ...
    @Test
    public void countDown_splitBrainProtection() {
        latch(0).countDown();
    }

    @Test(expected = SplitBrainProtectionException.class)
    public void countDown_noSplitBrainProtection() {
        latch(3).countDown();
    }

    @Test
    public void trySetCount_splitBrainProtection() {
        latch(0).trySetCount(1);
    }

    @Test(expected = SplitBrainProtectionException.class)
    public void trySetCount_noSplitBrainProtection() {
        latch(3).trySetCount(1);
    }

    protected ICountDownLatch latch(int index) {
        return latch(index, splitBrainProtectionOn);
    }
}
