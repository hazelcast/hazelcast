...
public class MessageTaskSecurityTest {
    ...
    private void assertGetRequiredPermission(String clsname) throws Exception {
        // metrics & management skipped
        if (SKIP_CLASS_MAP.containsKey(clsname) || clsname.startsWith("com.hazelcast.client.impl.protocol.task.m")) {
            return;
        }
    ...
    private static void skip(Class<?> classToSkip, String reason) {
        assertTrue(reason != null && !reason.isEmpty());
