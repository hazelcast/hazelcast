...
public abstract class RestApiConfigTestBase extends AbstractTextProtocolsTestBase {
    ...
    protected static final TestUrl[] TEST_URLS = new TestUrl[]{
            new TestUrl(CLUSTER_WRITE, POST, "/hazelcast/rest/management/security/permissions", "HTTP/1.1 403 Forbidden"),
            ...
            new TestUrl(WAN, POST, "/hazelcast/rest/wan/clearWanQueues", "HTTP/1.1 403 Forbidden"),
            new TestUrl(WAN, POST, "/hazelcast/rest/wan/addWanConfig", "HTTP/1.1 403 Forbidden"),
            new TestUrl(WAN, POST, "/hazelcast/rest/wan/pausePublisher", "HTTP/1.1 403 Forbidden"),
            new TestUrl(WAN, POST, "/hazelcast/rest/wan/stopPublisher", "HTTP/1.1 403 Forbidden"),
            new TestUrl(WAN, POST, "/hazelcast/rest/wan/resumePublisher", "HTTP/1.1 403 Forbidden"),
            new TestUrl(WAN, POST, "/hazelcast/rest/wan/consistencyCheck/map", "HTTP/1.1 403 Forbidden"),
            new TestUrl(WAN, POST, "/hazelcast/rest/wan/addWanConfig", "HTTP/1.1 403 Forbidden"),
