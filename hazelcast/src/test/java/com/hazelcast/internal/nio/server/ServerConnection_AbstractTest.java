...
public abstract class ServerConnection_AbstractTest extends HazelcastTestSupport {
    ...
    protected ServerNetworkingService networkingServiceA;
    protected ServerNetworkingService networkingServiceB;
    protected ServerNetworkingService networkingServiceC;
    ...
    public void setup() throws Exception {
        ...
        logger = loggingService.getLogger(ServerConnection_AbstractTest.class);
    ...
    protected ServerNetworkingService newNetworkingService(MetricsRegistry metricsRegistry) throws Exception {
        ...
        ServerNetworkingService serverNetworkingService = new ServerNetworkingService(null,
                ioService,
                registry,
    ...
    protected ServerConnection connect(Address address) {
        return connect(networkingServiceA, address);
    }

    protected ServerConnection connect(final ServerNetworkingService service, final Address address) {
        service.getEndpointManager(MEMBER).getOrConnect(address);

        final AtomicReference<ServerConnection> ref = new AtomicReference<>();
        assertTrueEventually(() -> {
            Connection c = service.getEndpointManager(MEMBER).getConnection(address);
            assertNotNull(c);
            ref.set((ServerConnection) c);
        });

        return ref.get();
    }

    public static ServerConnection getConnection(ServerNetworkingService service, SocketAddress localSocketAddress) {
        long startMs = System.currentTimeMillis();

        for (; ; ) {
            for (ServerConnection connection : service.getEndpointManager(MEMBER).getActiveConnections()) {
