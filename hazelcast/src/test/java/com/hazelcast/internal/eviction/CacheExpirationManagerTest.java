...
public class CacheExpirationManagerTest extends ExpirationManagerTest {
    ...
    public void clearExpiredRecordsTask_should_not_be_started_if_member_is_lite() {
        ...
        Cache cache = cacheManager.createCache("test", cacheConfig);
        cache.put(1, 1);
    ...
    private void backgroundClearTaskStops_whenLifecycleState(LifecycleEvent.LifecycleState lifecycleState) {
        ...
        Cache cache = cacheManager.createCache("test", cacheConfiguration);
        cache.put(1, 1);
    ...
    public static class SimpleEntryListenerConfiguration implements CacheEntryListenerConfiguration {

        private SimpleEntryListener listener;

        public SimpleEntryListenerConfiguration(SimpleEntryListener listener) {
            this.listener = listener;
        }


        @Override
        public Factory<SimpleEntryListener> getCacheEntryListenerFactory() {
            return FactoryBuilder.factoryOf(listener);
        }
