...
public class AbstractNearCacheRecordStoreTest {
    ...
    public void testRecordCreation_withReservation() {
        ...
        long reservationId1 = store.tryReserveForUpdate(KEY);
        long reservationId2 = store.tryReserveForUpdate(KEY);
        ...
        assertNull(store.tryPublishReserved(KEY, keyData, VALUE2, reservationId2, true));
        assertRecordState(reservationId1);

        // can publish the value with the correct reservation ID
        assertEquals(VALUE1, store.tryPublishReserved(KEY, keyData, VALUE1, reservationId1, true));
        assertRecordState(READ_PERMITTED);

        // cannot change a published value with the wrong reservation ID
        assertEquals(VALUE1, store.tryPublishReserved(KEY, keyData, VALUE2, reservationId2, true));
        assertRecordState(READ_PERMITTED);

        // cannot change a published value with the correct reservation ID
        assertEquals(VALUE1, store.tryPublishReserved(KEY, keyData, VALUE2, reservationId1, true));
        assertRecordState(READ_PERMITTED);
