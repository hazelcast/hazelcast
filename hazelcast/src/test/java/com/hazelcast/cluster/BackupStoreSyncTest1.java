 ...
 * @author yueyulin
 */
public class BackupStoreSyncTest1 {

    private HazelcastInstance[] hs;
    private String[] ids = new String[5];

    public BackupStoreSyncTest1() {
    }
    ...
    public String readKeyFile(String file) {
        ...
        try {
            ...
            Logger.getLogger(BackupStoreSyncTest1.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            try {
                br.close();
            } catch (Exception ex) {
    ...
    public void testPutDelete() {
        ...
        for (int i = 0; i < dataSize; i++) {
            ...
            for (int j = 0; j < 5; j++) {
                String bkDir = dataDir + "idSync" + j + "/" + partitionId + "/backup/";
