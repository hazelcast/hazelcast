...
public class SplitBrainAddMemberTest extends SplitBrainTestSupport {
    ...
    protected void onAfterSplitBrainCreated(HazelcastInstance[] firstHalf, HazelcastInstance[] secondHalf) {
        ...
        // add a new member to the second brain only
        HazelcastInstance newMember = createHazelcastInstanceInBrain(1);
        waitAllForSafeState(secondHalf);
        for (HazelcastInstance hz : firstHalf) {
            assertClusterSize(FIRST_HALF, hz);
        }
        for (HazelcastInstance hz : secondHalf) {
            assertClusterSize(SECOND_HALF + 1, hz);
        }
        newMember.shutdown();
        waitAllForSafeState(secondHalf);
    }
