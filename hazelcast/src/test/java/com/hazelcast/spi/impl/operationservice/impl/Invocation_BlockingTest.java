...
//        int threadId = 1;
//        Operation op = new LockOperation(new InternalLockNamespace(key), nodeEngine.toData(key), threadId, -1, 10 * callTimeout)
//                .setPartitionId(partitionId);
...
//        }
    ...
    public void sync_testWaitingIndefinitely() throws InterruptedException {
        final Config config = new Config().setProperty(OPERATION_CALL_TIMEOUT_MILLIS, "3000");
    ...
    public void sync_testWaitingInfinitelyForTryLock() throws InterruptedException {
        ...
        spawn(new Runnable() {
            ...
            public void run() {
                try {
                    boolean result = lock.tryLock(10, SECONDS);
                    latch.countDown();
                } catch (Exception ignored) {
                    ignored.printStackTrace();
                }
