...
public class OperationRunnerImplTest extends HazelcastTestSupport {
    ...
    @Test
    public void runOperation_whenRunThrowsOOME() {

    }

    @Test
    public void runOperation_whenNoResponse_and_forceSyncEnabled(){
        final AtomicLong counter = new AtomicLong();
        ...
        setForceSync(op, true);
        op.setResponseHandler(responseHandler);

        operationRunner.run(op);

        assertEquals(1, counter.get());
        verify(responseHandler).sendResponse(any(ForcedSyncResponse.class));
    }

    @Test
    public void runOperation_whenNoResponse_and_forceSyncDisabled(){
        final AtomicLong counter = new AtomicLong();
    ...
    @Test
    public void runOperation_whenBackupAware_and_hasResponse_and_noSyncBackups_and_forceSyncEnabled(){
        final AtomicLong counter = new AtomicLong();
        final Object result = "foo";
        ...
        op.setResponseHandler(responseHandler);
        setCallerAddress(op, getAddress(local));

        operationRunner.run(op);

        assertEquals(1, counter.get());
        NormalResponse expected = new NormalResponse(result, op.getCallId(), 0, op.isUrgent());
        verify(responseHandler).sendResponse(eq(expected));
    }

    @Test
    public void runOperation_whenBackupAware_and_hasResponse_and_forceSyncDisabled(){
        final AtomicLong counter = new AtomicLong();
        final Object result = "foo";
        DummyBackupAwareOperation op = new DummyBackupAwareOperation() {
            @Override
        ...
        assertEquals(1, counter.get());
        NormalResponse expected = new NormalResponse(result, op.getCallId(), 1, op.isUrgent());
        verify(responseHandler).sendResponse(eq(expected));
    }

    @Test
    public void runOperation_whenBackupAware_and_hasNoResponse_and_forceSyncEnabled(){
        final AtomicLong counter = new AtomicLong();
        DummyBackupAwareOperation op = new DummyBackupAwareOperation() {
            @Override
            public void run() throws Exception {
        ...
        op.returnsResponse = false;
        op.asyncBackupCount = 1;
        op.setResponseHandler(responseHandler);
        op.setPartitionId(operationRunner.getPartitionId());
        setForceSync(op, true);
        setCallerAddress(op, getAddress(local));
        ...
        assertEquals(1, counter.get());
        NormalResponse expected = new NormalResponse(null, op.getCallId(), 1, op.isUrgent());
        verify(responseHandler).sendResponse(eq(expected));
    }

    @Test
    public void runOperation_whenBackupAware_and_hasNoResponse_and_forceSyncDisabled(){
