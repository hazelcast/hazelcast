...
public abstract class AbstractScheduledExecutorQuorumTest {
    ...
    protected static ScheduledExecutorConfig newExecConfig(QuorumType quorumType, String quorumName, String postfix) {
        ScheduledExecutorConfig config = new ScheduledExecutorConfig(EXEC_NAME + quorumType.name() + postfix);
    ...
    protected static void initCluster(Config config, TestHazelcastInstanceFactory factory, QuorumType... types) {
        ...
        for (QuorumType quorumType : types) {
            ...
            for (String postfix : asList("", "shutdown", "shutdownNow")) {
                ...
                ScheduledExecutorConfig execConfig = newExecConfig(quorumType, quorumName, postfix);
                config.addQuorumConfig(quorumConfig);
                config.addScheduledExecutorConfig(execConfig);
            }
        }
    ...
    protected IScheduledExecutorService exec(int index, QuorumType quorumType) {
        return exec(index, quorumType, "");
    }

    protected IScheduledExecutorService exec(int index, QuorumType quorumType, String postfix) {
        return cluster.instance[index].getScheduledExecutorService(EXEC_NAME + quorumType.name() + postfix);
    }
