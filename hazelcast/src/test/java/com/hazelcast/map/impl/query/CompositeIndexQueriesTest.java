...
public class CompositeIndexQueriesTest extends HazelcastTestSupport {
    ...
    public void before() {
        ...
        ss = getNode(instance).getSerializationService();
        extractors = Extractors.newBuilder(ss).build();
    }
    ...
    private void check(String sql, int expectedSize, int... queryCounts) {
        if (sql != null) {
            ...
            for (Map.Entry<Integer, Person> entry : result) {
                QueryEntry queryEntry = new QueryEntry(ss, ss.toData(entry.getKey()), entry.getValue(), extractors);
                assertTrue(sqlPredicate.apply(queryEntry));
            }
