    ...
    public void txn_map_contains_newly_put_key_even_it_is_null_cached_after_addition() throws Exception {
        ...
        Thread txnThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                Object result = instance.executeTransaction(new TransactionalTask<Object>() {
                    @Override
                ...
                keyExistInTxnMap.set((Boolean) result);
            }
        });
        txnThread.start();


        Thread nullCachingThread = new Thread(new Runnable() {
            @Override
            public void run() {
                ...
                instance.getMap(mapName).get(1);

                waitNullCaching.countDown();
            }
        });
        nullCachingThread.start();

        assertJoinable(txnThread, nullCachingThread);
