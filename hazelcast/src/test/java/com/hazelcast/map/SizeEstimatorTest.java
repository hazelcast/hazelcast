...
public class SizeEstimatorTest extends HazelcastTestSupport {
    ...
    public void testNearCache() {
        String NO_NEAR_CACHED_MAP = randomString();
        String NEAR_CACHED_MAP = randomString();
        ...
        IMap<String, String> noNearCached = h[0].getMap(NO_NEAR_CACHED_MAP);
        for (int i = 0; i < 1000; i++) {
            noNearCached.put("key" + i, "value" + i);
        }

        IMap<String, String> nearCachedMap = h[0].getMap(NEAR_CACHED_MAP);
        for (int i = 0; i < 1000; i++) {
