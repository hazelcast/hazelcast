 ...
 * <ol>
 *     <li>Concurrency tasks: so tasks that are offered by other threads</li>
...
public abstract class Eventloop implements Executor {
    ...
    public final CircularQueue<Runnable> localQueue;

    protected final PriorityQueue<ScheduledTask> scheduledQueue;
    ...
    protected final MpmcArrayQueue concurrentQueue;
    ...
    protected Eventloop(EventloopFactory eventloopFactory, EventloopType type) {
        ...
        this.localQueue = new CircularQueue<>(eventloopFactory.localRunQueueCapacity);
        this.concurrentQueue = new MpmcArrayQueue(eventloopFactory.concurrentRunQueueCapacity);
        this.scheduledQueue = new BoundPriorityQueue<>(eventloopFactory.scheduledTaskQueueCapacity);
        scheduler.init(this);
            ...
            switch (oldState) {
                ...
                case RUNNING:
                    if (STATE.compareAndSet(this, oldState, SHUTDOWN)) {
                        concurrentQueue.add(SHUTDOWN_TASK);
                        wakeup();
                        return;
                    }
                    break;
     ...
     * @throws InterruptedException if the threas was interrupted while waiting.
     */
    ...
    public final boolean offer(Runnable task) {
        ...
        } else if (concurrentQueue.offer(task)) {
            wakeup();
            return true;
        } else {
            return false;
    ...
    protected final boolean runScheduledTasks() {
        for (int k = 0; k < batchSize; k++) {
            ScheduledTask scheduledTask = scheduledQueue.peek();
    ...
    protected boolean runConcurrentTasks() {
        for (int k = 0; k < batchSize; k++) {
            Object task = concurrentQueue.poll();
            if (task == null) {
                // there are no more tasks
                return false;
    ...
    public abstract class Unsafe {
        ...
        public boolean schedule(Runnable command, long delay, TimeUnit unit) {
            ...
            return scheduledQueue.offer(scheduledTask);
            // return fut;
        }

        public boolean scheduleWithFixedDelay(Runnable command, long initialDelay, long delay, TimeUnit unit) {
            checkNotNull(command);
            ...
            scheduledTask.delayNanos = delayNanos;
            return scheduledQueue.offer(scheduledTask);
        }

        public boolean scheduleAtFixedRate(Runnable command, long initialDelay, long period, TimeUnit unit) {
            checkNotNull(command);
            checkNotNegative(initialDelay, "initialDelay");
            ...
            if (periodNanos < -1) {
                periodNanos = Long.MAX_VALUE;
            }
            scheduledTask.periodNanos = periodNanos;
            return scheduledQueue.offer(scheduledTask);
        }
