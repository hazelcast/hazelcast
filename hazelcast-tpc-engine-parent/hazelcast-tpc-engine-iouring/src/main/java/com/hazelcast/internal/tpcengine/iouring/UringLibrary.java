...
public final class UringLibrary {
    ...
    static {
        ...
        try {
            if (!OS.isLinux()) {
                throw new UnsupportedOperationException("Linux is required, but found OS [" + OS.osName() + "]");
            }
            ...
            if (!isValidLinuxKernel()) {
                throw new UnsupportedOperationException("Linux >= " + MIN_MAJOR_VERSION + "." + MIN_MINOR_VERSION
                        + " is required, but found version [" + OS.osVersion() + "]");
            }
    ...
    private static boolean isValidLinuxKernel() {
        if (OS.linuxKernelMajorVersion() < MIN_MAJOR_VERSION) {
            return false;
        } else if (OS.linuxKernelMajorVersion() == MIN_MAJOR_VERSION && OS.linuxKernelMinorVersion() < MIN_MINOR_VERSION) {
            return false;
        } else {
            return true;
        }
