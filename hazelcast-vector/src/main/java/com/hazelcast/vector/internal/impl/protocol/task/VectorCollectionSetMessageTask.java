...
public class VectorCollectionSetMessageTask
        extends AbstractVectorCollectionPartitionMessageTask<VectorCollectionSetCodec.RequestParameters> {
    VectorCollectionSetMessageTask(ClientMessage clientMessage, Node node, Connection connection) {
        super(clientMessage, node, connection);
    }

    @Override
    protected Operation prepareOperation() {
        return new SetOperation(parameters.name, parameters.key, parameters.value.getValue(), parameters.value.getVectors())
                .setPartitionId(getPartitionId());
    }

    @Override
    protected VectorCollectionSetCodec.RequestParameters decodeClientMessage(ClientMessage clientMessage) {
        return VectorCollectionSetCodec.decodeRequest(clientMessage);
    }

    @Override
    protected Object processResponseBeforeSending(Object response) {
        recordStats(LocalVectorCollectionStatsImpl::incrementSetLatencyNanos);
        return response;
    }

    @Override
    protected ClientMessage encodeResponse(Object response) {
        return VectorCollectionSetCodec.encodeResponse();
    }
