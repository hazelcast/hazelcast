...
public class ClientMapReduceProxy
    ...
    private ClientMessage invoke(ClientMessage request, String jobId) throws Exception {
        ...
        if (trackableJob != null) {
            ClientConnection clientConnection = trackableJob.clientInvocation.getSendConnectionOrWait();
            Address runningMember = clientConnection.getEndPoint();
            ClientInvocation clientInvocation = new ClientInvocation(getClient(), request, runningMember);
    ...
    private ClientMessage getRequest(String name, String jobId, Collection keys,
        ...
        if (keyValueSource instanceof MapKeyValueSource) {
            ...
            return MapReduceForMapCodec.encodeRequest(name, jobId, predicateData, mapperData, combinerFactoryData,
                    reducerFactoryData, source.getMapName(), chunkSize, list, topologyChangedStrategyName);
        ...
        return MapReduceForCustomCodec.encodeRequest(name, jobId, predicateData, mapperData, combinerFactoryData,
                reducerFactoryData, toData(keyValueSource), chunkSize, list, topologyChangedStrategyName);
