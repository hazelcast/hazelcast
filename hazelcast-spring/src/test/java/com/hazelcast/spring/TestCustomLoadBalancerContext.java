...
public class TestCustomLoadBalancerContext {
    ...
    public void testCustomLoadBalancer() {
        ...
        assertTrue(loadBalancer1 instanceof DummyLoadBalancer);
        assertEquals("default-name", ((DummyLoadBalancer) loadBalancer1).getName());

        ClientConfig config2 = client2.getClientConfig();
        LoadBalancer loadBalancer2 = config2.getLoadBalancer();
        assertTrue(loadBalancer2 instanceof DummyLoadBalancer);
        assertEquals("custom-balancer-name", ((DummyLoadBalancer) loadBalancer2).getName());
    }
}
