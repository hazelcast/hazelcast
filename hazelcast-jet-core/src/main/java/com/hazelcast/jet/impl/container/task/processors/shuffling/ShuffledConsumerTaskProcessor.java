...
public class ShuffledConsumerTaskProcessor extends ConsumerTaskProcessor {
    ...
    private final Map<Consumer, Integer> markersCache = new IdentityHashMap<Consumer, Integer>();
    ...
    private void initSenders(ContainerContextImpl containerContext, int taskID, boolean receiver) {
        ...
        if (!receiver) {
            for (Address address : jobManager.getJobContext().getSocketWriters().keySet()) {
                ...
                jobManager.registerShufflingSender(taskID, containerContext, address, sender);
            }
        }
    }
    ...
    private void initCalculationStrategies(Set<CalculationStrategy> strategies,
        ...
        JobManager jobManager = this.containerContext.getJobContext().getJobManager();
