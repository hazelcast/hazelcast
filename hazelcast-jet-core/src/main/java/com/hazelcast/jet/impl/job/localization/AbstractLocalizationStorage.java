...
import com.hazelcast.jet.impl.job.localization.classloader.JobClassLoader;
...
public abstract class AbstractLocalizationStorage<S> implements LocalizationStorage {
    ...
    protected final JobConfig jetConfig;

    private ClassLoader classLoader;

    private volatile boolean accepted;

    public AbstractLocalizationStorage(JobConfig config) {
    ...
    public void accept() throws InvalidLocalizationException {
        ...
        AccessController.doPrivileged(new PrivilegedAction<Void>() {
            @Override
            public Void run() {
                classLoader = new JobClassLoader(AbstractLocalizationStorage.this);
                return null;
            }
        });
    }
