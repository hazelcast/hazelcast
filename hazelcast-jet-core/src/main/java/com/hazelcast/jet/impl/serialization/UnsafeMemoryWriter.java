...
public class UnsafeMemoryWriter implements MemoryWriter {
    ...
    UnsafeMemoryWriter(boolean reverse) {
    ...
    public void writeInt(byte[] bytes, int offset, int value) {
        MEM.putInt(bytes, ARRAY_BYTE_BASE_OFFSET + offset, reverse ? Integer.reverseBytes(value) : value);
    }

    @Override
    public void writeLong(byte[] bytes, int offset, long value) {
        MEM.putLong(bytes, ARRAY_BYTE_BASE_OFFSET + offset, reverse ? Long.reverseBytes(value) : value);
    }
}
