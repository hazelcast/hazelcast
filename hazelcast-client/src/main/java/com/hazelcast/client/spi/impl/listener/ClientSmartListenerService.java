...
public class ClientSmartListenerService extends ClientListenerServiceImpl
    ...
    private void invoke(Member member, ClientRegistrationKey registrationKey) {
        ...
        ClientInvocation invocation =
                new ClientInvocation(client, registrationKey.getHandler(), request, member.getAddress());
        try {
            ...
            ClientEventRegistration registration
                    = new ClientEventRegistration(serverRegistrationId, request.getCallId(), sendConnection);
    ...
    private void invoke(Connection connection, ClientRegistrationKey registrationKey) {
        ...
        ClientRequest request = registrationKey.getRequest();
        ClientInvocation invocation =
                new ClientInvocation(client, registrationKey.getHandler(), request, connection);
        try {
            ClientInvocationFuture future = invocation.invoke();
            String serverRegistrationId = serializationService.toObject(future.get());
