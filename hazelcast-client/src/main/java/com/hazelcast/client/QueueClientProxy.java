...
public class QueueClientProxy<E> extends AbstractQueue<E> implements IQueue<E> {
    ...
    public void destroy() {
        proxyHelper.doCommand(Command.DESTROY, new String[]{InstanceType.QUEUE.name(), getName()});
    }
    ...
    public java.util.Iterator<E> iterator() {
        ...
        if (protocol.hasBuffer()) {
            for (ByteBuffer bb : protocol.buffers) {
                list.add((E) proxyHelper.toObject(new Data(SerializationConstants.CONSTANT_TYPE_BYTE_ARRAY, bb.array())));
            }
        }
