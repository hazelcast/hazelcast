...
public class ClientRingbufferProxy<E> extends ClientProxy implements Ringbuffer<E> {
    ...
    public ICompletableFuture<Long> addAsync(E item, OverflowPolicy overflowPolicy) {
        ...
        try {
            ClientInvocationFuture invocationFuture = new ClientInvocation(getClient(), request, getName(), partitionId, false).invoke();
    ...
    protected <T> T invoke(ClientMessage clientMessage, int partitionId) {
        try {
            final Future future = new ClientInvocation(getClient(), clientMessage, getName(), partitionId, false).invoke();
            return (T) future.get();
