...
final class TransactionProxy {
    ...
    void begin() {
        try {
            ...
            txnId = sendAndReceive(new CreateTransactionRequest(options, (int) threadId, sXid));
            state = ACTIVE;
        } catch (Exception e) {
            closeConnection();
    ...
    public void prepare() {
        try {
            ...
            checkThread();
            checkTimeout();
            sendAndReceive(new PrepareTransactionRequest((int) threadId));
            state = PREPARED;
        } catch (Exception e) {
            state = ROLLING_BACK;
            closeConnection();
    ...
    void commit(boolean prepareAndCommit) {
        try {
            ...
            checkThread();
            checkTimeout();
            sendAndReceive(new CommitTransactionRequest((int) threadId, prepareAndCommit));
            state = COMMITTED;
        } catch (Exception e) {
            state = ROLLING_BACK;
            throw ExceptionUtil.rethrow(e);
