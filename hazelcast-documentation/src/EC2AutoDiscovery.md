...
Hazelcast supports EC2 Auto Discovery. EC2 Auto Discovery is useful when you cannot or do not want to provide the list of possible IP addresses. To configure your cluster to use EC2 Auto Discovery, you disable join over multicast and TCP/IP, you enable AWS, and you provide your credentials (access and secret keys). 

You also need to add *hazelcast-cloud.jar* dependency to your project. Note that it is also bundled inside *hazelcast-all.jar*. The Hazelcast cloud module does not depend on any other third party modules.
...
The parameter `region` specifies where the members are running. Its default value is `us-east-1`. If the cluster is running on a different region, you must specify it with the parameter `region`. Otherwise, the cluster will not be formed since the members will not discover each other.
...
![image](images/NoteSmall.jpg) ***NOTE***: *When you create your EC2 security group ensure that you have the relevant TCP/IP ports open.  In Hazelcast's case the default ports are 5701 and upwards as you add more instances to a machine.*
...
To make sure EC2 instances are found correctly, you can use the `AWSClient` class. It determines the private IP addresses of EC2 instances to be connected. Just give the values of the parameters you specified in the `aws` element to this class, as shown below. The example below also prints to show whether your EC2 instances are found.
