...
- `eviction-policy`: The eviction policy **(currently available on Hi-Density storage only)** defines which entries are evicted (removed) from the cache when the cache is low in space. Its default value is `RANDOM`. The following eviction policies are available:
