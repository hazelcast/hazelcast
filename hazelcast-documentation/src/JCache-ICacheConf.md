...
- `eviction-policy`: The eviction policy **(currently available on High-Density Memory Store only)** defines which entries are evicted (removed) from the cache when the cache is low in space. Its default value is `NONE`. The following eviction policies are available:
